...
export default defineEventHandler(async event => {
  ...
  const { error: authorizationError } = await serviceRole.from('auth_invitations').upsert({
  ...
  if (authorizationError)
    throw createError({ statusCode: 500, message: 'Não foi possível autorizar o convite.' })

  const { data, error } = await serviceRole.auth.admin.inviteUserByEmail(normalizedEmail, {
    data: { full_name: body.fullName },
    redirectTo,
  })

  if (error) {
    await serviceRole.from('auth_invitations').delete().eq('email', normalizedEmail)
